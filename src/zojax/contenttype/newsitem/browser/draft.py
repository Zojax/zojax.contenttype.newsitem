...
from zojax.contenttype.newsitem.interfaces import _


class NewsCategoryStep(WizardStepForm):

    title = _(u'Category')


class AddNewsCategory(PageletForm):
    ...
    def update(self):
        ...
        ct = getUtility(IContentType, 'contenttype.newsitem.category').__bind__(category)

        self.ct = ct
        super(AddNewsCategory, self).update()
